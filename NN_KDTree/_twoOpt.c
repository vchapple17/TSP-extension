...
static PyObject *twoOpt_twoOpt( PyObject * self, PyObject * args ) {
  ...
  /* Make results a Python Object */
  /* https://stackoverflow.com/questions/3498210/returning-a-tuple-of-multipe-objects-in-python-c-api */
  PyObject *dist_obj = Py_BuildValue("l", dist);
  PyObject *route = PyList_New(n);
  for (int i = 0; i < n; i++ ) {
    PyList_SetItem(route, i, PyInt_FromLong(c[i]));
  }
  PyObject *ret = PyTuple_New(2);
  PyTuple_SetItem(ret, 0, dist_obj);
  PyTuple_SetItem(ret, 1, route);
  return ret;
}
